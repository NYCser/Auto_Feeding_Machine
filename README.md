...
1. Khởi tạo các ngoại vi (GPIO, ADC, I2C, UART, PWM).
2. Đọc giá trị nhiệt độ từ DS18B20 và giá trị Analog từ cảm biến độ đục.
3. Tính toán lượng thức ăn còn lại dựa trên khoảng cách từ HC-SR04.
4. Đóng gói dữ liệu thành chuỗi (String) và gửi sang ESP32 qua UART.
5. Lắng nghe lệnh từ UART để điều khiển Servo hoặc bật/tắt đèn.

ESP32 (Lập trình qua Arduino IDE):

1. Quản lý kết nối Wi-Fi và duy trì trạng thái kết nối.
2. Nhận chuỗi dữ liệu từ STM32 qua UART, phân tách (parse) dữ liệu.
3. Thực hiện chức năng cầu nối (UART-to-Network bridge) để chuyển tiếp dữ liệu.
